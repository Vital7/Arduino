...
template <typename ServerType>
String ESP8266WebServerTemplate<ServerType>::getCompressedContentType(const String& fileName, const String& contentType)
{
  using namespace mime;

  String defaultMimeType = String(FPSTR(mimeTable[none].mimeType));
  for (int i = gz; i < maxType; i++) {
    Entry entry = compressedTypes[i];
    String mimeType = String(FPSTR(entry.mimeType));
    if (fileName.endsWith(String(FPSTR(entry.endsWith))) &&
        contentType != mimeType) &&
        contentType != defaultMimeType) {
          return mimeType.substring(14);
    }
  }
...
void ESP8266WebServerTemplate<ServerType>::_streamFileCore(const size_t fileSize, const String &fileName, const String &contentType)
{
  ...
  String type = getCompressedContentType(fileName, contentType);
  if (type != NULL) {
    sendHeader(F("Content-Encoding"), type);
  }
